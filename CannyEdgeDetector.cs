...
namespace CannyEdgeDetection
{
	...
	public static class CannyEdgeDetector
	{
        ...
        {
            originalimg = sourceBitmap;
            afterGaussImg = ApplyFilter(ToGrayscale(originalimg), gaussKernel);
            ...
            unsafe
            {
                ...
                for (int i = 1; i < bitmapData1.Height-1; i++)
                {
                    for (int j = 1; j < bitmapData1.Width-1; j++)
                    {
                        ...
                        if ((gradientDirections[i, j] > Math.PI / 1.5 && (imagePointer1[-bitmapData1.Stride] > imagePointer1[0] || imagePointer1[bitmapData1.Stride] > imagePointer1[0])) ||
                            (gradientDirections[i, j] > Math.PI / 8 && (imagePointer1[-bitmapData1.Stride] > imagePointer1[0] || imagePointer1[bitmapData1.Stride] > imagePointer1[0])) ||
            ...
            unsafe
            {
                ...
                for (int i = 0; i < bitmapData1.Height; i++)
                {
                    for (int j = 0; j < bitmapData1.Width; j++)
                    {
                        ...
                        gradientDirections[i, j] = Math.Atan(imagePointer2[0] / (imagePointer1[0] > 0 ? imagePointer1[0] : 0.01)); // Считаем по красному, все равно черно-белая картинка.
